...
def send_sendgrid_emails(emails):
    ...
    for e in emails:
        ...
        if not (
            response.status_code == requests.codes.ok or response.status_code == requests.codes.accepted):
            all_errors = [error_obj['message'] for error_obj in
...
def sendgrid_send_email(to_address, subject, body):
    ...
    response = requests.request('POST', constants.SENDGRID_URL,
